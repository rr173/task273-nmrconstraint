...
func TestExemptPersistsAndDuplicateConflicts(t *testing.T) {
	h, st := newAPI(t)
	batchID, cons := seedTriangle(t, st)
	if _, err := st.diag.Solve(context.Background(), batchID); err != nil {
		t.Fatal(err)
	}
	sets, err := st.diag.BuildConflictSets(batchID)
	if err != nil {
		t.Fatal(err)
	}
	target := cons[2].ID
	rr := doJSON(t, h, http.MethodPost, fmt.Sprintf("/api/batches/%d/exemptions", batchID), map[string]any{
		"constraint_id": target, "reason": "peak overlap",
	}, nil)
	if rr.Code != http.StatusCreated {
		t.Fatalf("exempt status=%d body=%s", rr.Code, rr.Body.String())
	}
	got, err := st.cons.Get(target)
	if err != nil {
		t.Fatal(err)
	}
	if got.Status != model.ConstraintExcluded {
		t.Fatalf("constraint status=%s want excluded", got.Status)
	}
	_, members, err := st.diag.GetConflictSet(sets[0].ID)
	if err != nil {
		t.Fatal(err)
	}
	cs, _, err := st.diag.GetConflictSet(sets[0].ID)
	if err != nil {
		t.Fatal(err)
	}
	_ = members
	if cs.Status != model.ConflictSetExempted {
		t.Fatalf("conflict set status=%s want exempted", cs.Status)
	}
	rr = doJSON(t, h, http.MethodPost, fmt.Sprintf("/api/batches/%d/exemptions", batchID), map[string]any{
		"constraint_id": target, "reason": "again",
	}, nil)
	if rr.Code != http.StatusConflict {
		t.Fatalf("duplicate exempt status=%d body=%s want 409", rr.Code, rr.Body.String())
	}
}
