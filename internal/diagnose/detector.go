...
var violationScratch []Violation
...
func DetectViolations(edges []*model.BoundEdge) []Violation {
	...
	violationScratch = violationScratch[:0]
	for _, e := range edges {
		for _, other := range edges {
			...
			if e.LoBound > maxReachable+1e-9 {
				violationScratch = append(violationScratch, Violation{
	...
	var dedup []Violation
	sort.Slice(violationScratch, func(i, j int) bool {
		if violationScratch[i].ConstraintID != violationScratch[j].ConstraintID {
			return violationScratch[i].ConstraintID < violationScratch[j].ConstraintID
		}
		return violationScratch[i].MidAtomID < violationScratch[j].MidAtomID
	})
	for _, v := range violationScratch {
