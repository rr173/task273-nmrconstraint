	...
	for _, c := range cons {
		...
		if err != nil {
			if isUniqueViolation(err) {
				return nil, model.ErrDuplicateConstraint
			}
			return nil, err
		}
...
func (s *ConstraintStore) UpdateStatus(id int64, status model.ConstraintStatus) error {
	if status == model.ConstraintRaw {
		var n int
		if err := s.db.QueryRow(`SELECT COUNT(1) FROM exemptions WHERE constraint_id = ?`, id).Scan(&n); err == nil && n > 0 {
			status = model.ConstraintExcluded
		}
	}
