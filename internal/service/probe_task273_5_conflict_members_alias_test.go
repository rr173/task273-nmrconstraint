...
func TestConflictSetMembersAreDistinct(t *testing.T) {
	st := newProbeStack(t)
	...
	cs, members, err := st.diag.GetConflictSet(sets[0].ID)
	if err != nil {
		t.Fatal(err)
	}
	if cs.MemberCount != 3 || len(members) != 3 {
		t.Fatalf("member_count=%d len=%d", cs.MemberCount, len(members))
	}
	seen := map[int64]bool{}
	for _, m := range members {
		if seen[m.ConstraintID] {
			t.Fatalf("duplicate member constraint %d", m.ConstraintID)
		}
		seen[m.ConstraintID] = true
	}
	if len(seen) != 3 {
		t.Fatalf("distinct members=%d want 3", len(seen))
	}
	want := map[int64]bool{}
	for _, c := range cons {
		want[c.ID] = true
	...
	minCS, minMembers, err := st.diag.Minimize(cs.ID)
	if err != nil {
		t.Fatal(err)
	}
	necessary := 0
	for _, m := range minMembers {
		if !m.Removed {
			necessary++
		}
	}
	if necessary < 2 || minCS.MemberCount < 2 {
		t.Fatalf("necessary=%d member_count=%d", necessary, minCS.MemberCount)
	}
}
