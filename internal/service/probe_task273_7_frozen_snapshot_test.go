...
func TestDraftSnapshotDoesNotTrackLiveBounds(t *testing.T) {
	st := newProbeStack(t)
	...
	_, before, err := st.snaps.Get(snap.ID)
	if err != nil {
		t.Fatal(err)
	}
	if len(before) != len(cons) {
		t.Fatalf("draft items=%d", len(before))
	}
	inverted := 0
	frozen := map[int64][2]float64{}
	for _, it := range before {
		frozen[it.ConstraintID] = [2]float64{it.LoBound, it.HiBound}
		if it.LoBound > it.HiBound+1e-9 {
			inverted++
		}
	}
	if inverted == 0 {
		t.Fatal("expected inverted bounds frozen in draft")
	}
	if _, err := st.diag.Exempt(batchID, cons[2].ID, "peak overlap"); err != nil {
		t.Fatal(err)
	}
	if _, err := st.diag.Solve(context.Background(), batchID); err != nil {
		t.Fatal(err)
	}
	_, after, err := st.snaps.Get(snap.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, it := range after {
		want, ok := frozen[it.ConstraintID]
		if !ok {
			t.Fatalf("missing constraint %d", it.ConstraintID)
		}
		if it.LoBound != want[0] || it.HiBound != want[1] {
			t.Fatalf("constraint %d bounds mutated %v,%v -> %v,%v", it.ConstraintID, want[0], want[1], it.LoBound, it.HiBound)
		}
		if it.LoBound > it.HiBound+1e-9 {
			stillInverted++
		}
	...
	if stillInverted == 0 {
		t.Fatal("frozen snapshot lost inverted evidence")
	}
}
