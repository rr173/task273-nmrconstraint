...
func TestConcurrentExemptSingleWinner(t *testing.T) {
	st := newProbeStack(t)
	...
	for i := 0; i < workers; i++ {
		...
		go func() {
			defer wg.Done()
			_, err := st.diag.Exempt(batchID, target, "peak overlap")
			if err == nil {
				okCh <- struct{}{}
				return
			}
			if !errors.Is(err, model.ErrExemptionExists) {
				errCh <- err
			}
		}()
	}
	...
	if len(okCh) != 1 {
		t.Fatalf("successes=%d want 1", len(okCh))
	}
	list, err := st.diag.Exemptions(batchID)
	if err != nil {
		t.Fatal(err)
	}
	if len(list) != 1 || list[0].ConstraintID != target {
		t.Fatalf("exemptions=%d", len(list))
	}
}
